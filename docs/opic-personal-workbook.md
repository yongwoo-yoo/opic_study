# 내 이야기로 준비하는 OPIc — 4카테고리 답변집

기준 자료: [오픽 시험지.pdf](오픽%20시험지.pdf), 기존 `transcripts/answer-worksheet-v2-clustered.md`의 한글 메모, `transcripts/scripts-final.md`의 영어 초안, IH 재생목록 자막 31편.

- [시험지 28쪽 질문 분류표](opic-question-map.md): 질문을 찾은 뒤 아래 답변 번호로 이동합니다.
- [자막 표현·전략 출처](opic-subtitle-notes.md): E01~E14의 뜻과 영상 시간을 확인합니다.
- **27개 답변**을 12문장으로 구성했습니다. 앞의 MP 3문장은 20초 안에 말할 수 있게 20~27단어로 유지했습니다. D09·D10·P05는 기존 영어 초안에만 있던 내용을 포함한 **확인용**입니다. 나머지도 감정·평가 표현은 한글 메모를 토대로 다듬은 제안이므로 본인 느낌에 맞춰 바꿉니다.
- 원래 메모와 스크립트는 보존했습니다. 자료가 없는 질문은 분류표에서 **보충**으로 표시했습니다. 별도 사건·인물·날짜를 만들어 경험을 채우지 않았습니다.

## 1. 답변 만드는 기준

MP는 **What → Why → Feeling** 순서로 연습합니다. 아래 **MP 세 문장 + 본문 + 마무리**를 이어 읽으면 완성 답변입니다. What/Why/Feeling이라는 라벨은 읽지 않습니다. 강조된 영어는 자막에서 확인한 표현을 본인 이야기로 바꿔 쓴 부분입니다.

| 카테고리 | MP에서 잡을 것 | 이후 구성 | 주의할 점 |
|---|---|---|---|
| D · 묘사 | 대상·핵심 특징 → 이유 → 감정 | 구체적 특징/예시 → 같은 핵심으로 마무리 | 대상 전체를 소개하되 초점을 하나 잡기 |
| H · 습관 | 반복 행동 → 이유 → 감정 | 언제·어떻게·누구와 → 선택적으로 짧은 비교 → 현재 습관 | 특정 하루의 사건으로 길게 빠지지 않기 |
| P · 과거 경험 | 사건 → 감정의 이유 → 당시 감정 | 필요한 배경 → 사건 진행 → 결과 → 의미 | 처음·최근·어릴 때 등 질문의 시간 조건 지키기 |
| C · 비교 | 현재 상태 → 이유 → 현재 감정 | 과거 → 현재의 차이 → 평가 | 같은 비교 기준 유지; A/B 비교면 같은 기준으로 두 대상 설명 |

강의 원문이 모든 유형에 이 순서를 강제하는 것은 아닙니다. 특히 Habit 강의는 행동 중심의 간단한 MP도 허용합니다. 여기서는 사용자와 합의한 What → Why → Feeling 순서로 통일했습니다. 표현 자체의 점수 효과를 보장하는 자료가 아니라, 핵심·이유·감정을 명확하게 말하기 위한 답변집입니다.

## 2. Description — 묘사

<a id="d01"></a>
### D01. 집과 가장 좋아하는 방 — 커피를 만드는 부엌

**시험지:** p.5 현재 집 / 좋아하는 방. **핵심:** 집 크기나 방 개수보다 부엌이 주는 즐거움. **메모:** 클러스터의 독립 주제.

**MP**

- **What:** Well, my favorite room is the kitchen.
- **Why:** You know, I make coffee and tea there.
- **Feeling:** So, I feel relaxed and happy in that room.

**본문**

I'm **such a coffee person**, so I spend a lot of time there. I usually grind my own beans first. Then I slowly make a fresh cup of coffee. Sometimes I make tea instead, you know. The smell of coffee fills the kitchen. **Whenever** that happens, I feel completely at home. I like taking a quiet moment before I drink it. Even a simple cup feels special when I make it myself.

**마무리**

**Overall**, that's why the kitchen is special to me.

**한글**

제가 가장 좋아하는 방은 부엌이에요. 그곳에서 커피와 차를 만들거든요. 그래서 부엌에 있으면 편안하고 행복해요. 저는 커피를 정말 좋아해서 부엌에서 많은 시간을 보내요. 보통 먼저 원두를 직접 갈고 천천히 신선한 커피를 내려요. 때로는 차를 만들기도 해요. 커피 향이 부엌에 퍼지면 집에 있다는 느낌이 들어요. 마시기 전 잠깐 조용한 시간을 갖는 것도 좋아해요. 직접 만들면 평범한 한 잔도 특별하게 느껴져요. 그래서 부엌이 저에게 특별해요.

**표현:** E04·E02·E03. **추가 질문:** 방 개수·구조를 명시적으로 물으면 실제 구조를 첫 문장 뒤에 한 문장 추가합니다.

<a id="d02"></a>
### D02. 좋아하는 음악 — 명상 음악

**시험지:** p.2 SET1 Q1 / SET2 Q1의 장르 부분. **핵심:** 복잡한 마음을 쉬게 해 주는 음악. **메모:** 클러스터 6.

**MP**

- **What:** Well, I love meditation music.
- **Why:** You know, it helps me **clear my head** after work.
- **Feeling:** So, I feel really calm when I listen to it.

**본문**

My head gets really busy during the week. I mean, there is always something to think about at work. But this music helps me slow down. It is quiet and very peaceful. I usually enjoy it with a cup of tea. That simple combination gives me a real break. I can focus on the music instead of my busy thoughts. After a while, I feel ready to rest.

**마무리**

**Overall**, it helps me **wind down and relax**.

**한글**

저는 명상 음악을 좋아해요. 퇴근 후 복잡한 머릿속을 정리하는 데 도움이 돼요. 들으면 마음이 차분해져요. 평일에는 일이 많아서 생각도 많아져요. 그런데 이 음악은 제 속도를 늦춰 줘요. 조용하고 평화로운 음악이에요. 보통 차 한 잔과 함께 즐기는데, 그 조합이 진짜 휴식이 돼요. 바쁜 생각 대신 음악에 집중할 수 있고, 조금 지나면 쉴 준비가 된 것 같아요. 그래서 긴장을 풀고 편안해지는 데 도움이 돼요.

**표현:** E08·E01. **보충:** 가수도 요구하는 문항에는 실제로 듣는 아티스트 한 명을 추가해야 합니다. 장르만으로 가수 부분까지 답했다고 보지 않습니다.

<a id="d03"></a>
### D03. 기억에 남는 해변 / 방문한 해외 장소 — 하와이

**시험지:** p.4 좋아하는·기억에 남는 해변; p.7 SET1 Q1 / SET2 Q1의 장소 부분. **핵심:** 자연을 가까이서 느끼는 곳. **메모:** 클러스터 4.

**MP**

- **What:** Well, a beach in Hawaii really stands out to me.
- **Why:** You know, I saw sea turtles there.
- **Feeling:** Honestly, that place still makes me excited.

**본문**

I went there on a camping trip with my wife. The beach was right near our campsite. We went snorkeling in the clear water. And, you know, we saw sea turtles in the ocean. There were also chickens walking around the campsite. I felt really close to nature the whole time. Seeing those animals so nearby made the place stand out. I still remember that feeling when I think of Hawaii.

**마무리**

**Put simply**, I'll never forget its beautiful nature.

**한글**

하와이의 한 해변이 특히 기억에 남아요. 그곳에서 바다거북을 봤거든요. 지금도 그 장소를 생각하면 설레요. 아내와 캠핑을 갔는데 해변이 캠핑장 바로 옆에 있었어요. 맑은 물에서 스노클링을 하다가 바다거북을 봤어요. 캠핑장에는 닭들도 돌아다녔어요. 여행 내내 자연이 가까이 느껴졌고, 동물들을 가까이서 본 게 특히 인상적이었어요. 하와이를 생각하면 그때의 느낌이 떠올라요. 그 아름다운 자연을 잊지 못할 거예요.

**표현:** E10. **맞춤 도입:** 국가 질문에는 “I visited the United States, specifically Hawaii.”로 시작합니다. 현지인 특징·해변 이름은 메모에 없으므로 보충합니다. ‘가장 최근’이나 ‘첫 해외여행’이라고 바꾸지 않습니다.

<a id="d04"></a>
### D04. 친구 묘사 — 꾸준함이 인상적인 친구

**시험지:** p.26 SET1~3 Q1. **핵심:** 꾸준함이라는 한 가지 특징. **메모:** 클러스터 5.

**MP**

- **What:** Well, my friend is a very consistent person.
- **Why:** You know, he always sticks to his routine.
- **Feeling:** Honestly, I really admire him for that.

**본문**

**The main thing is**, he never gives up easily. Once he starts something, he keeps going. He also follows his routine every single day. **If I'm honest**, that kind of consistency is hard for me. I sometimes stop when things get difficult. So, you know, he makes me want to improve my own habits. I notice the difference when I think about my own routine. His example reminds me to keep trying.

**마무리**

That's what I'd really like to learn from him.

**한글**

제 친구는 정말 꾸준한 사람이에요. 언제나 자기 일과를 지키거든요. 그래서 그 친구가 정말 존경스러워요. 무엇보다 쉽게 포기하지 않아요. 일단 시작하면 계속하고, 매일 꾸준히 해요. 솔직히 그런 꾸준함은 저에게 어려워요. 저는 힘들면 가끔 멈추기도 해요. 제 일과를 돌아보면 그 차이가 느껴져요. 그 친구를 보면 계속 노력해야겠다는 생각이 들어요. 저도 그 점을 배우고 싶어요.

**표현:** E05·E09. **보충:** 건강한 사람 질문(p.17)에는 이 친구의 실제 운동·식습관이 필요합니다. 꾸준하다는 사실만으로 운동을 한다고 만들지 않습니다.

<a id="d05"></a>
### D05. 날씨와 계절 — 가을

**시험지:** p.27 SET1~3 Q1. **핵심:** 밖에서 취미를 즐기기 좋은 계절. **메모:** 기존 스크립트 D.

**MP**

- **What:** Well, fall is my favorite season in Korea.
- **Why:** You know, it's perfect for coffee and camping.
- **Feeling:** So, I feel really happy when fall comes.

**본문**

The weather is cool and comfortable. It is not as hot as summer. It is not freezing like winter either. I can drink coffee outside and enjoy the fresh air. I can also go camping without worrying about the heat. **The main thing is**, I can really enjoy being outdoors. I look forward to those simple outdoor moments. That is why the season fits my hobbies so well.

**마무리**

**Overall**, fall is just perfect for me.

**한글**

한국에서 제가 가장 좋아하는 계절은 가을이에요. 커피를 마시고 캠핑하기에 좋거든요. 가을이 오면 행복해요. 날씨가 선선하고 쾌적해요. 여름처럼 덥지도 않고 겨울처럼 춥지도 않아요. 밖에서 커피를 마시며 신선한 공기를 즐길 수 있어요. 더위를 걱정하지 않고 캠핑도 할 수 있죠. 그런 소소한 야외 시간이 기다려져요. 제 취미와 잘 맞는 계절이라서 가을이 저에게 딱 좋아요.

**표현:** E05. **보충:** ‘오늘 날씨’는 실제 날씨를 넣어 따로 답합니다. 가을 이야기로 오늘의 상태를 대신하지 않습니다.

<a id="d06"></a>
### D06. 산업 / 회사 — 내가 일하는 반도체 분야

**시험지:** p.12 SET1 Q1, SET4 Q1의 관심 업계 소개 부분. **핵심:** 내 일과 직접 연결된 산업. **메모:** 독립 주제의 회사·데이터 처리 업무.

**MP**

- **What:** Well, I'd like to talk about Korea's semiconductor industry.
- **Why:** You know, I work in that industry and handle data.
- **Feeling:** So, it's very interesting to me.

**본문**

I work for a semiconductor company in Korea. My job is to process and analyze data. So, you know, I deal with this industry every day. I also see semiconductor news quite often. **The main thing is**, it is part of my real working life. It is much more than just a topic in the news for me. When I read about the industry, I think about my own work. That personal connection keeps me interested.

**마무리**

**Overall**, I feel closely connected to this industry.

**한글**

한국의 반도체 산업에 대해 이야기하고 싶어요. 제가 그 분야에서 일하며 데이터를 다루거든요. 그래서 저에게 흥미로운 주제예요. 저는 한국의 반도체 회사에서 데이터를 처리하고 분석해요. 매일 이 산업을 접하고 관련 뉴스도 자주 봐요. 저에게 반도체는 뉴스 속 주제 이상의 실제 직장 생활이에요. 산업 뉴스를 읽으면 제 업무가 떠올라요. 그런 개인적인 연결 때문에 계속 관심이 가요. 그래서 이 산업과 가깝게 느껴져요.

**표현:** E05. **보충:** 회사명·성공 과정·3년 전과의 산업 비교는 별도 자료가 필요합니다. 기존 메모의 메모리 공급 전망을 현재의 검증된 사실로 옮기지 않았습니다.

<a id="d07"></a>
### D07. 명절 소개 — 추석

**시험지:** p.21 휴일 종류 소개 부분. **핵심:** 맛있는 음식과 가족. **메모:** 독립 주제, 기존 스크립트 X.

**MP**

- **What:** Well, my favorite Korean holiday is Chuseok.
- **Why:** You know, my family gets together and shares good food.
- **Feeling:** So, it's a really happy time for me.

**본문**

It is kind of like Korean Thanksgiving. My family gets together for the holiday. We sit down and share a big meal. There is always a lot of delicious food. After eating, we talk and catch up with one another. **Put simply**, family and food make the day special. I especially enjoy the time we spend at the table. Being together makes the meal even better.

**마무리**

**Overall**, that's why I love Chuseok.

**한글**

제가 가장 좋아하는 한국 명절은 추석이에요. 가족이 모여 맛있는 음식을 나누기 때문이에요. 저에게 정말 행복한 시간이죠. 추석은 한국의 추수감사절 같은 명절이에요. 가족이 모여 푸짐한 식사를 해요. 맛있는 음식이 많고, 식사 후에는 서로 근황을 이야기해요. 특히 식탁에 함께 앉아 있는 시간이 좋아요. 가족과 함께하니 음식도 더 맛있게 느껴져요. 그래서 추석을 좋아해요.

**표현:** E10. **보충:** 여러 명절의 종류를 요구하면 추석 외 명절도 짧게 추가합니다.

<a id="d08"></a>
### D08. 카페 도안 — 보조 소재

**시험지:** 카페 자체 문항은 없음. p.25 자유시간 장소 질문에 개인 사례로 활용. **메모:** 클러스터 1.

**MP**

- **What:** Well, my favorite café is Cafe Doan.
- **Why:** You know, its coffee has really clear flavors.
- **Feeling:** So, every visit is exciting for me.

**본문**

I'm **such a coffee person**, so good beans matter to me. The café uses beans from famous roasters. Some are from Korea, and others are from abroad. Each cup has a different taste and aroma. I go there so often that I became friends with the owner. Sometimes, you know, he even gives me beans to enjoy at home. I like noticing how one cup differs from another. That makes every visit interesting to me.

**마무리**

**Overall**, this café makes my hobby more enjoyable.

**한글**

제가 가장 좋아하는 카페는 카페 도안이에요. 커피 맛이 아주 선명하거든요. 갈 때마다 설레요. 저는 커피를 정말 좋아해서 좋은 원두가 중요해요. 이곳은 국내외 유명 로스터리의 원두를 사용해요. 커피마다 맛과 향이 달라서 그 차이를 느끼는 재미가 있어요. 자주 가다 보니 사장님과 친구가 되었고, 집에서 즐기라며 원두를 주실 때도 있어요. 매번 방문이 흥미롭고 커피 취미가 더 즐거워져요.

**표현:** E04·E03. **수정:** 기존 “coffee beans from Korea”는 한국산 생두처럼 들릴 수 있어 국내외 **로스터리** 원두로 바로잡았습니다. 식당 질문에는 음식 제공 여부부터 확인해야 합니다.

<a id="d09"></a>
### D09. 국내 여행지 — 제주도 [확인용]

**시험지:** p.6 SET1~3 Q1. **자료 상태:** 기존 스크립트 M에 있으나 원래 클러스터 메모에는 없는 내용입니다. 제주 방문·렌터카·오름·흑돼지가 실제 경험인지 확인 후 사용합니다.

**MP**

- **What:** Well, my favorite place in Korea is Jeju Island.
- **Why:** You know, it has beautiful beaches and volcanic hills.
- **Feeling:** So, I really feel like I'm on vacation there.

**본문**

Jeju feels very different from the mainland. I rented a car and drove around the island. That was a great way to see different places. I visited a beautiful beach and climbed an oreum. An oreum is a small volcanic hill, by the way. I also tried the local black pork, and it was delicious. Each stop showed me a different side of the island. I enjoyed the scenery as much as the food.

**마무리**

**Overall**, I love Jeju's unique atmosphere.

**한글**

한국에서 제가 가장 좋아하는 곳은 제주도예요. 아름다운 해변과 오름이 있어서요. 그곳에 가면 진짜 휴가를 온 것 같아요. 제주는 본토와 분위기가 많이 달라요. 렌터카를 빌려 섬을 돌아다니며 여러 장소를 봤어요. 아름다운 해변에 가고 작은 화산 언덕인 오름에도 올랐어요. 유명한 흑돼지도 맛있었어요. 들르는 곳마다 제주의 다른 모습을 볼 수 있었고, 음식만큼 풍경도 좋았어요. 그 독특한 분위기가 마음에 들어요.

**표현:** E03. **보충:** p.18 한국 지형 전체를 물으면 이 답변 앞에 전국의 지형 특징을 설명해야 합니다. 제주 사례만으로 전국 설명을 대신하지 않습니다.

<a id="d10"></a>
### D10. 좋아하는 옷 — 편안함 [확인용]

**시험지:** p.28 SET1 Q2. **자료 상태:** 티셔츠·청바지 취향은 기존 스크립트 Q에만 있습니다.

**MP**

- **What:** Well, I usually wear T-shirts and jeans.
- **Why:** You know, they're simple and comfortable.
- **Feeling:** Honestly, I feel like myself in those clothes.

**본문**

**If I'm honest**, I do not follow fashion trends very much. I usually choose a simple T-shirt first. Then I wear it with a comfortable pair of jeans. That outfit is easy to wear almost anywhere. **The main thing is** being able to move around comfortably. So, you know, I keep my style simple. I feel more comfortable when I do not have to think much about my clothes. That is why I often choose the same kind of outfit.

**마무리**

**Overall**, comfort matters most to me.

**한글**

저는 보통 티셔츠와 청바지를 입어요. 단순하고 편하거든요. 그런 옷을 입으면 제 모습 그대로인 것 같아요. 솔직히 유행을 많이 따르지는 않아요. 먼저 간단한 티셔츠를 고르고 편한 청바지를 입어요. 어디에나 입기 쉽고 활동하기도 편해요. 옷에 대해 많이 고민하지 않아도 돼서 마음이 편해요. 그래서 비슷한 옷차림을 자주 선택해요. 저에게는 편안함이 가장 중요해요.

**표현:** E09·E05. **보충:** 한국 사람 전체의 패션, 어릴 때 유행, 실제 쇼핑 습관은 별도 답변입니다.

## 3. Habit — 습관

<a id="h01"></a>
### H01. 음악 듣는 습관 — 주말의 차와 명상 음악

**시험지:** p.2 SET1 Q2. **메모:** 클러스터 6.

**MP**

- **What:** Well, I listen to meditation music every weekend.
- **Why:** You know, it helps me **clear my head** after work.
- **Feeling:** So, I feel really relaxed while listening to it.

**본문**

I usually make a cup of tea first. Then I put on some meditation music. I sit quietly and enjoy both at the same time. My mind is often busy during the week. But, you know, this routine helps me slow down. It is how I **wind down and relax**. I look forward to that quiet time on weekends. Even a short break like this feels refreshing.

**마무리**

**Overall**, it's my favorite weekend routine.

**한글**

저는 주말마다 명상 음악을 들어요. 퇴근 후 복잡한 머릿속을 정리하는 데 도움이 되거든요. 들으면서 정말 편안해져요. 보통 먼저 차를 만들고 음악을 틀어요. 조용히 앉아서 차와 음악을 함께 즐겨요. 평일에는 생각이 많지만 이 습관 덕분에 속도를 늦출 수 있어요. 주말의 그 조용한 시간이 기다려지고, 짧게 쉬어도 기분이 상쾌해져요. 제가 가장 좋아하는 주말 습관이에요.

**표현:** E08·E01. **보충:** 장소·기기를 요구하면 실제 듣는 곳과 기기를 추가합니다. 메모에는 정확한 장소·기기가 없습니다.

<a id="h02"></a>
### H02. 집에서 하는 일 — 커피 내리기

**시험지:** p.5 SET1 Q2 / SET5 Q2의 주말 일과. **메모:** 독립 주제, 기존 스크립트 A·B.

**MP**

- **What:** Well, I make my own coffee every weekend.
- **Why:** You know, I enjoy the whole process.
- **Feeling:** So, it makes me feel relaxed and happy.

**본문**

First, I choose the coffee beans I want to use. Then I grind them in my kitchen. I take my time while making the coffee. Sometimes I use beans from my favorite café. The owner occasionally shares some with me, you know. **The main thing is**, I enjoy making the whole drink myself. I like the smell of the beans as I prepare them. Taking my time is part of the fun.

**마무리**

**Overall**, it's my favorite thing to do at home.

**한글**

저는 주말마다 직접 커피를 내려요. 그 과정 전체를 즐기기 때문이에요. 커피를 만들면 편안하고 행복해져요. 먼저 쓸 원두를 골라 부엌에서 갈아요. 서두르지 않고 천천히 커피를 만들어요. 가끔은 좋아하는 카페의 원두를 써요. 사장님이 원두를 나눠 주실 때도 있거든요. 준비할 때 나는 원두 향도 좋고, 천천히 만드는 것 자체가 재미있어요. 집에서 가장 좋아하는 활동이에요.

**표현:** E05. **보충:** 평일과 주말을 모두 묻는 경우 실제 평일 일과도 추가합니다. 집안일 질문에는 청소·설거지 등 실제 가사 행동이 필요합니다.

<a id="h03"></a>
### H03. 건강을 위해 하는 일 — 야외 러닝

**시험지:** p.17 SET1 Q2. **메모:** 독립 주제의 러닝, 기존 스크립트 I.

**MP**

- **What:** Well, I run outside to stay active.
- **Why:** You know, I love the fresh air and the wind.
- **Feeling:** Honestly, running makes me feel free.

**본문**

**If I'm honest**, the gym is not really my thing. I would rather exercise outside. I can feel the wind while I am running. I also get to enjoy the fresh morning air. Of course, running is good exercise for me too. So, you know, I get exercise and outdoor time together. Being outside makes the run more enjoyable. Afterward, my mind feels clearer.

**마무리**

**Overall**, running keeps me active and clears my head.

**한글**

저는 건강을 위해 밖에서 달려요. 신선한 공기와 바람이 좋거든요. 달릴 때 자유로운 기분이 들어요. 솔직히 헬스장은 저와 잘 맞지 않아서 밖에서 운동하는 편이 좋아요. 달리며 바람과 아침 공기를 느낄 수 있어요. 운동도 되고 야외 시간도 가질 수 있죠. 밖에 있으니 달리기가 더 즐겁고, 끝나고 나면 머릿속도 맑아져요. 그래서 러닝은 몸을 움직이고 생각을 정리하는 데 도움이 돼요.

**표현:** E09·E08. **적용 범위:** 건강 관리 질문에는 적합하지만 ‘헬스장 시설 묘사’에는 그대로 사용하지 않습니다. 러닝 시작 시기·횟수·거리는 실제 값이 필요합니다.

<a id="h04"></a>
### H04. 인터넷 습관 — 재테크 공부와 뉴스

**시험지:** p.9 SET1 Q2 / SET2~3 Q1 / SET4 Q2. **메모:** 독립 주제.

**MP**

- **What:** Well, I use the internet for financial news.
- **Why:** You know, I want to learn more about investing.
- **Feeling:** So, I find it really interesting.

**본문**

**These days**, I check stock market news online. I also read about money and investing. If I find something new, I look it up. That helps me understand the news better. I mean, there is always more to learn about this topic. So checking financial information has become part of my routine. One article often leads me to another question. That is what keeps the topic interesting.

**마무리**

**Overall**, the internet helps me keep learning.

**한글**

저는 인터넷으로 금융 뉴스를 봐요. 투자에 대해 더 배우고 싶거든요. 그래서 정말 흥미로워요. 요즘 온라인으로 주식 시장 소식을 확인하고 돈과 투자에 관한 글도 읽어요. 새로운 내용을 발견하면 찾아보는데, 뉴스를 이해하는 데 도움이 돼요. 이 주제는 배울 게 계속 있어요. 기사 하나가 또 다른 궁금증으로 이어지기도 해요. 그래서 금융 정보를 확인하는 것이 일상의 일부가 되었고, 인터넷은 제 공부에 도움이 돼요.

**표현:** E13. **보충:** 주로 보는 동영상·좋아하는 사이트·어제 한 일은 각각 실제 채널·사이트·행동을 넣어야 합니다.

<a id="h05"></a>
### H05. 약속 잡는 습관 — 주말 위주로 일정 잡기

**시험지:** p.24 SET1~2 Q1의 예약 종류, SET1 Q2의 방법 일부. **메모:** 독립 주제, 자동차 수리 예약.

**MP**

- **What:** Well, I usually make appointments for weekends.
- **Why:** You know, I'm too busy on weekdays.
- **Feeling:** So, weekend appointments feel less stressful.

**본문**

For example, I sometimes need a car repair appointment. I first check which weekend is free. Then I choose a time that fits my schedule. **The main thing is** finding a time I can actually keep. Of course, something unexpected can still come up. When that happens, I call the shop and change the appointment. I try to let them know as soon as I can. Having a clear plan makes the appointment easier for me.

**마무리**

**Overall**, weekends work best for me.

**한글**

저는 보통 주말에 약속을 잡아요. 평일에는 너무 바빠서 주말 약속이 덜 부담스러워요. 예를 들어 자동차 수리 예약이 필요할 때 먼저 시간이 되는 주말을 확인해요. 그런 다음 제 일정에 맞는 시간을 골라요. 지킬 수 있는 시간을 찾는 게 중요해요. 예상치 못한 일이 생기면 정비소에 가능한 빨리 연락해서 예약을 바꿔요. 일정이 명확하면 마음이 편해서 저에게는 주말이 가장 좋아요.

**표현:** E05. **보충:** 실제 예약 경로(전화·앱 등)를 본문 앞에 추가하면 ‘방법’ 답변이 완성됩니다. 치과·미용실로 지정하면 자동차 정비소로 바꾸지 않습니다.

<a id="h06"></a>
### H06. 명절에 하는 일 — 가족과 먹고 이야기하기

**시험지:** p.21 휴일 활동 문항. **메모:** 기존 스크립트 X.

**MP**

- **What:** Well, my family eats and talks together on Chuseok.
- **Why:** You know, it's a rare chance to be together.
- **Feeling:** So, I feel really happy during the holiday.

**본문**

I always look forward to the holiday food. My family sits down and eats together. We talk about what has been happening in our lives. Sometimes we stay at the table for a long time. But **the main thing is** spending time together. You know, it is a nice chance for everyone to catch up. I enjoy listening to everyone's stories. Those conversations are part of what I remember about the holiday.

**마무리**

**Overall**, family time makes Chuseok special.

**한글**

추석에는 가족과 함께 먹고 이야기해요. 다 같이 모일 흔치 않은 기회라서 행복해요. 저는 명절 음식이 늘 기대돼요. 가족이 둘러앉아 식사하며 서로 어떻게 지냈는지 이야기해요. 가끔은 식탁에 오래 앉아 있기도 해요. 하지만 가장 중요한 건 함께 시간을 보내는 거예요. 가족들의 이야기를 듣는 게 좋고, 그 대화가 추석의 기억으로 남아요. 가족과 보내는 시간이 추석을 특별하게 만들어요.

**표현:** E05. **적용 범위:** p.26 가족과 주로 하는 일에는 ‘추석에 가족을 만날 때’라는 실제 범위를 밝힙니다. 최근 명절·어릴 때 추석 경험으로 바꾸려면 해당 사건이 필요합니다.

<a id="h07"></a>
### H07. 은행 업무 — 휴대폰으로 송금하기

**시험지:** p.22 SET1 Q2의 은행 업무 내용에 조건부 활용. **메모:** 기존 스크립트 R.

**MP**

- **What:** Well, I do most of my banking on my phone.
- **Why:** You know, I can send money in seconds.
- **Feeling:** So, banking feels much easier now.

**본문**

**These days**, I rarely visit a bank branch. I usually open the banking app on my phone. Then I check my account or send money. The whole process takes only a few seconds. **The main thing is**, I do not need to make a separate trip. You know, I can handle it wherever I am. That saves me time during a busy day. I like being able to finish a simple task right away.

**마무리**

**Overall**, mobile banking is really convenient.

**한글**

저는 은행 업무를 대부분 휴대폰으로 해요. 몇 초 만에 송금할 수 있어서 지금은 훨씬 쉬워졌어요. 요즘은 은행 지점에 거의 가지 않아요. 휴대폰으로 은행 앱을 열어 계좌를 확인하거나 돈을 보내요. 과정이 몇 초밖에 걸리지 않고 따로 은행에 갈 필요도 없어요. 어디서든 처리할 수 있어 바쁜 날 시간을 아껴 줘요. 간단한 일을 바로 끝낼 수 있어 모바일 뱅킹이 정말 편리해요.

**표현:** E13·E05. **주의:** 질문이 실제 ‘은행 지점에 가서 하는 순서’를 요구하면 창구 업무 경험을 보충합니다. 앱 이야기만으로 현장 절차까지 답한 것은 아닙니다.

<a id="h08"></a>
### H08. 휴대폰 활용 — 버스 도착 정보 확인

**시험지:** p.10 SET1 Q2, p.11 SET1 Q2의 기술 사용 예시. **메모:** 기존 스크립트 O.

**MP**

- **What:** Well, I check bus times on my phone.
- **Why:** You know, the app shows the bus in real time.
- **Feeling:** So, I feel much more at ease.

**본문**

Before leaving, I open the bus app on my phone. It shows me where the bus is. It also tells me when the bus will arrive. So I know when to walk to the bus stop. **Put simply**, I do not have to guess anymore. You know, that makes waiting much easier. I can check the information before I step outside. That helps me feel more prepared for the trip.

**마무리**

**Overall**, this app makes bus travel easier.

**한글**

저는 휴대폰으로 버스 시간을 확인해요. 앱에서 버스 위치를 실시간으로 보여 줘서 마음이 한결 놓여요. 나가기 전에 앱을 열면 버스가 어디 있는지, 언제 도착하는지 알 수 있어요. 그래서 언제 정류장으로 걸어갈지 정할 수 있죠. 밖에 나가기 전 미리 정보를 확인하니 막연히 추측할 필요가 없어요. 기다리기도 쉬워지고 이동할 준비도 더 잘할 수 있어요. 이 앱 덕분에 버스 타기가 편해요.

**표현:** E10. **보충:** ‘매일 가장 자주 쓰는 기술’이라는 빈도를 충족하는지는 확인합니다. ‘가장 좋아하는 기능’으로 쓰려면 첫 문장을 선호 문장으로 바꿉니다.

## 4. Past Experience — 과거 경험

<a id="p01"></a>
### P01. 기술 문제 — 세 번 만에 고장 난 이어폰

**시험지:** p.11 SET2 Q3; 전자기기 구매 문제 질문에 재사용. **메모:** 클러스터 2.

**MP**

- **What:** Well, my new earphones broke after only three uses.
- **Why:** You know, I bought them from overseas.
- **Feeling:** So, I was really frustrated and worried.

**본문**

I had bought the earphones for running. But they stopped working almost right away. **I couldn't believe** they broke after only three uses. At first, I thought overseas service would be difficult. Luckily, the shopping platform offered a return option. I sent them back and received a full refund. The return process was easier than I expected. That made a frustrating experience end well.

**마무리**

**Overall**, I was relieved, and I trusted the app more.

**한글**

새 이어폰이 세 번 사용한 뒤 고장 났어요. 해외에서 산 제품이라서 정말 답답하고 걱정됐어요. 러닝할 때 쓰려고 샀는데 거의 바로 작동을 멈췄어요. 세 번 만에 고장 났다는 게 믿기지 않았어요. 처음에는 해외 제품의 사후 서비스가 어려울 줄 알았는데, 다행히 쇼핑 플랫폼에서 반품할 수 있었어요. 반품하고 전액 환불받았고, 절차도 생각보다 쉬웠어요. 답답했던 경험이 잘 끝나서 안도했고 앱을 더 믿게 됐어요.

**표현:** E12. **경계:** 가구 고장·전화기 고장·은행 문제를 묻는 문항에는 이 사건을 바꿔 붙이지 않습니다. p.12의 ‘관심 산업에서 기대에 못 미친 제품’은 실제 질문 범위가 소비자 전자기기를 포함할 때만 연결합니다.

<a id="p02"></a>
### P02. 여행·해변 경험 — 하와이의 동물들

**시험지:** p.4 기억에 남는 경험; p.7 SET2 Q3 / SET3 Q3. **메모:** 클러스터 4.

**MP**

- **What:** Well, I saw amazing animals while camping in Hawaii.
- **Why:** You know, chickens and sea turtles were all around us.
- **Feeling:** Honestly, I was amazed by the nature there.

**본문**

The chickens walked freely around the campsite. That was really unusual to me. In Korea, I usually see chickens inside an enclosure. We also went snorkeling in the ocean. Then, you know, we saw sea turtles in the water. **I couldn't believe** how close all the animals were. I kept thinking about how different it felt from home. Those encounters made the trip especially memorable.

**마무리**

**Overall**, it became one of my favorite trips.

**한글**

하와이에서 캠핑하다 멋진 동물들을 봤어요. 캠핑장 주변의 닭과 바다거북을 보고 자연에 감탄했어요. 닭들이 캠핑장을 자유롭게 돌아다니는 모습은 저에게 낯설었어요. 한국에서는 보통 닭을 우리 안에서 보거든요. 바다에서 스노클링을 할 때는 바다거북도 봤어요. 동물들이 그렇게 가까이에 있다는 게 믿기지 않았어요. 집에서의 모습과 얼마나 다른지 계속 생각났고, 그 만남들이 여행을 특별하게 만들었어요. 제가 좋아하는 여행 중 하나가 됐어요.

**표현:** E12·E03. **선택적 직접화법:** “I thought to myself, ‘This is so different from what I'm used to.’”는 당시 느낌을 재구성한 연습 문장입니다(E11). 실제 기억과 맞을 때만 본문에 넣습니다.

**시간 조건:** 첫 해외여행·어릴 때 여행·최근 여행이라고 확인되지 않았습니다. p.26 가족과 특별했던 경험에는 ‘아내와 함께한 캠핑’으로 도입하고 같은 실제 사건을 사용합니다.

<a id="p03"></a>
### P03. 음악에 관심을 갖게 된 계기 — 상점에서 들은 음악

**시험지:** p.2 SET1 Q3 / SET2 Q2의 계기 부분. **메모:** 클러스터 6.

**MP**

- **What:** Well, I first heard meditation music in a small shop.
- **Why:** You know, it made me relax right away.
- **Feeling:** Honestly, I felt surprisingly calm.

**본문**

I stopped and listened to the music for a while. It gave the whole shop a peaceful mood. I really liked the way it made me feel. After that day, I looked for similar music. Then I started playing it while drinking tea on weekends. Now it helps me **wind down and relax** after work. I still remember how calm I felt in that shop. A simple moment turned into a habit I enjoy.

**마무리**

**Overall**, that moment started my weekend habit.

**한글**

작은 가게에서 처음 명상 음악을 들었어요. 바로 긴장이 풀려서 뜻밖에 차분한 기분이 들었어요. 잠시 멈춰 음악을 들었는데 가게 전체가 평화롭게 느껴졌어요. 그 느낌이 좋아서 그날 이후 비슷한 음악을 찾았어요. 그러다 주말에 차를 마시며 듣기 시작했죠. 지금은 퇴근 후 긴장을 푸는 데 도움이 돼요. 가게에서 느꼈던 차분함이 아직도 기억나요. 그 짧은 순간이 제가 좋아하는 주말 습관의 시작이었어요.

**표현:** E01. **복합형:** 취향 변화까지 물으면 C03의 과거·현재 대조를 짧게 붙입니다. 라이브 공연을 들은 경험이 아니므로 p.2 SET2 Q3에는 사용하지 않습니다.

<a id="p04"></a>
### P04. 예약 변경 — 자동차 수리 일정 바꾸기

**시험지:** p.24 SET2 Q3. **메모:** 클러스터 8, 기존 스크립트 Y.

**MP**

- **What:** Well, I once changed a car repair appointment.
- **Why:** You know, something came up at the last minute.
- **Feeling:** So, I felt really sorry about it.

**본문**

I called the repair shop as soon as I could. First, I explained that something unexpected had happened. Then I said, “Could I come another day?” The staff listened and checked another date for me. Luckily, they were very kind about the change. In the end, they moved my appointment without any problem. I appreciated how understanding they were. After the call, I felt less worried about the repair.

**마무리**

**Overall**, I was relieved that it worked out.

**한글**

자동차 수리 예약을 바꾼 적이 있어요. 갑자기 일이 생겨서 정말 미안했어요. 최대한 빨리 정비소에 전화해 예상치 못한 일이 생겼다고 설명했어요. 그런 다음 다른 날 가도 되는지 물었어요. 직원이 다른 날짜를 확인해 줬고 다행히 변경에도 친절했어요. 결국 문제없이 예약을 옮겼어요. 이해해 줘서 고마웠고 통화 후에는 수리 걱정도 줄었어요. 일이 잘 풀려서 안심했어요.

**표현:** E05의 과거형 응용. **선택적 직접화법:** “Could I move my appointment to another day?”는 기존 롤플레이를 이용한 재구성 문장입니다. 실제 대화의 녹취는 아닙니다. 치과·미용실 예약으로 지정된 질문에는 별도 사건이 필요합니다.

<a id="p05"></a>
### P05. 국내 여행 경험 — 제주도의 풍경 [확인용]

**시험지:** p.6 SET1 Q3 / SET3 Q3, p.18 국내 장소 추억. **자료 상태:** D09와 같은 기존 초안의 제주 경험. 실제 기억과 일치하는지 확인합니다.

**MP**

- **What:** Well, I remember a road trip around Jeju Island.
- **Why:** You know, I visited a beach and climbed an oreum.
- **Feeling:** So, it felt like a different world.

**본문**

I rented a car and drove around the island. First, I went to a beautiful beach. I also climbed an oreum, which is a small volcanic hill. The scenery felt very different from the mainland. Later, I tried Jeju's famous black pork. It was delicious, and it made the trip even better. I liked seeing different scenery as I moved around. The beach and the hill both stayed in my memory.

**마무리**

**Overall**, it felt like a real getaway.

**한글**

제주도를 차로 여행했던 일이 기억나요. 해변에 가고 오름에 올라서 다른 세상에 온 듯했어요. 렌터카를 빌려 섬을 돌아다녔어요. 먼저 아름다운 해변에 가고, 작은 화산 언덕인 오름에도 올랐어요. 풍경이 본토와 아주 다르게 느껴졌어요. 나중에 먹은 제주 흑돼지도 맛있어서 여행이 더 좋았어요. 이동하면서 여러 풍경을 볼 수 있었고 해변과 오름 모두 기억에 남아요. 제대로 여행을 떠나온 기분이었어요.

**표현:** E03. **보충:** 어릴 때 여행·최근 여행·여행 중 문제와 해결이라는 조건은 현재 자료로 충족되지 않습니다.

## 5. Comparison — 비교

<a id="c01"></a>
### C01. 은행의 과거와 현재 — 방문에서 앱으로

**시험지:** p.22 SET1 Q3 / SET2 Q2. **비교 기준:** 업무를 처리하는 방법. **메모:** 기존 스크립트 R, 클러스터 3.

**MP**

- **What:** Well, I do my banking on my phone these days.
- **Why:** You know, I don't need to visit a bank.
- **Feeling:** So, banking feels much easier now.

**본문**

In the past, I had to visit a bank more often. I even went there to update my bankbook. That took extra time and planning. Now, I can send money in just a few seconds. I only need to open an app on my phone. **The main thing is**, I can do it almost anywhere. The same task feels much simpler today. I no longer have to plan a trip for a quick transfer.

**마무리**

**Overall**, mobile banking is much more convenient.

**한글**

요즘은 휴대폰으로 은행 업무를 해요. 은행에 갈 필요가 없어서 지금은 훨씬 쉬워졌어요. 예전에는 은행에 더 자주 갔고 통장을 정리하러 가기도 했어요. 시간과 계획이 필요했죠. 이제는 휴대폰 앱만 열면 몇 초 만에 송금할 수 있어요. 거의 어디서나 할 수 있고, 같은 일이 훨씬 간단하게 느껴져요. 간단한 송금 때문에 외출을 계획할 필요도 없어요. 모바일 뱅킹이 훨씬 편리해요.

**표현:** E13·E05. **보충:** ‘어렸을 때’라는 시점을 정확히 요구하면 당시 은행에 갔던 실제 기억을 확인합니다. 단순한 과거를 임의로 어린 시절로 바꾸지 않습니다.

<a id="c02"></a>
### C02. 주문·예약 방식의 변화 — 직접 방문에서 앱으로

**시험지:** p.11 기술의 과거·현재; p.15 외식 문화 변화 중 주문 방식. **메모:** 클러스터 3.

**MP**

- **What:** Well, ordering is mostly digital these days.
- **Why:** You know, we can use apps and kiosks.
- **Feeling:** Honestly, I find that really convenient.

**본문**

In the past, we usually ordered directly from a staff member. We also made many reservations in person. Now, we can tap a kiosk at a restaurant. We can also order or make a reservation with an app. So, you know, fewer steps require face-to-face help. **Put simply**, the whole process has become faster. I notice this change whenever I place an order. Using a screen now feels like a normal part of the process.

**마무리**

**Overall**, screens have changed how we order.

**한글**

요즘은 주문 방식이 대부분 디지털이에요. 앱과 키오스크를 쓸 수 있어서 정말 편리해요. 예전에는 주로 직원에게 직접 주문하고 예약도 방문해서 했어요. 이제는 식당에서 키오스크를 누르거나 앱으로 주문하고 예약할 수 있어요. 직접 도움을 받아야 하는 단계가 줄어 전체 과정이 빨라졌어요. 주문할 때마다 이런 변화를 느껴요. 화면을 쓰는 것이 이제는 자연스러운 과정이 되었어요. 화면이 주문 방식을 바꿨어요.

**표현:** E13·E10. **문항별 조정:** 음식점 질문에서는 예약·다른 업종을 줄이고 음식 주문만 설명합니다. 건강식 메뉴 변화·식품 구매의 정확한 20년 변화는 다른 질문입니다.

<a id="c03"></a>
### C03. 음악 취향·생활의 변화 — 없던 주말 습관이 생김

**시험지:** p.2 SET1 Q3 / SET2 Q2의 변화 부분; p.25 개인 자유시간 변화에 응용. **메모:** 클러스터 6의 ‘그 경험 후 취미가 생겼다’.

**MP**

- **What:** Well, meditation music is now my weekend routine.
- **Why:** You know, it helps me relax with a cup of tea.
- **Feeling:** So, I'm really happy I found it.

**본문**

Before, I did not have a special weekend routine. Then I heard meditation music in a small shop. I noticed that the music made me feel calm. After that, I started looking for similar music. Now, I play it while drinking tea every weekend. So, you know, music has become part of the way I rest. The quiet music gives my weekends a different feeling. I am glad that I kept listening after that first day.

**마무리**

**Overall**, I now have a quiet way to **unwind**.

**한글**

이제 명상 음악은 제 주말 습관이에요. 차와 함께 들으면 긴장이 풀려서 음악을 알게 된 게 기뻐요. 예전에는 특별한 주말 습관이 없었어요. 그러다 작은 가게에서 명상 음악을 들었고 마음이 차분해지는 걸 느꼈어요. 이후 비슷한 음악을 찾기 시작했어요. 지금은 주말마다 차를 마시며 듣고, 음악이 제 휴식의 일부가 되었어요. 조용한 음악 덕분에 주말이 다르게 느껴져요. 처음 들은 뒤 계속 듣기를 잘했다는 생각이 들어요. 이제는 조용히 긴장을 푸는 방법이 생겼어요.

**표현:** E05·E06. **경계:** 예전에 록이나 팝을 좋아했다고 만들지 않았습니다. 두 음악 장르 비교 문항에는 두 번째 장르와 차이점이 필요합니다. 자유시간 전체 비교라면 ‘음악을 듣는 시간’에 초점을 좁힌다는 문장을 먼저 붙입니다.

<a id="c04"></a>
### C04. 교통 정보의 변화 — 막연한 기다림에서 실시간 확인으로

**시험지:** p.11 기술 비교에 활용. p.19의 ‘어릴 때 교통수단’은 비교를 직접 요구하지 않으므로 아래 설명 참고. **메모:** 기존 스크립트 O.

**MP**

- **What:** Well, I can now track buses on my phone.
- **Why:** You know, I can see when my bus will arrive.
- **Feeling:** So, I feel much more relaxed while waiting.

**본문**

In the past, people just waited at the bus stop. They did not know exactly when the bus would arrive. That made waiting feel uncertain. Now, I can check the bus location on my phone. The app shows the information in real time. **The main thing is**, I do not have to keep guessing. I can decide when to head to the stop. Knowing the arrival time makes the wait feel shorter.

**마무리**

**Overall**, taking the bus is easier now.

**한글**

이제 휴대폰으로 버스 위치를 확인할 수 있어요. 버스가 언제 오는지 알 수 있어서 기다리는 동안 더 편안해요. 예전에는 정류장에서 막연히 기다렸고 정확한 도착 시간을 몰랐어요. 그래서 기다림이 불확실하게 느껴졌죠. 지금은 앱으로 버스 위치를 실시간 확인할 수 있어요. 언제 정류장으로 갈지 정할 수 있고 더는 추측하지 않아도 돼요. 도착 시간을 아니까 기다리는 시간도 짧게 느껴져요. 이제 버스 타기가 더 쉬워요.

**표현:** E05. **과거 묘사로 변형:** p.19에서는 실제 어린 시절 이용 수단을 먼저 밝히고 과거 모습을 설명합니다. 현재 앱 비교는 마지막 한 문장 정도만 선택적으로 붙입니다. 어린 시절 경험은 별도 확인이 필요합니다.

## 6. 자료가 없는 질문을 준비하는 방법

분류표의 **보충**은 스킵 추천이나 점수 판단이 아니라, 현재 메모만으로 완성할 수 없다는 뜻입니다. 다음 틀에서 대괄호를 실제 내용으로 채웁니다. 빈칸이 남은 문장은 완성 답변이 아닙니다.

### D 틀 — 영화·호텔·가구·식당 등

**What:** My favorite [대상] is [이름/구체적 대상].

**Why:** The main reason is [좋아하는 특징과 이유].

**Feeling:** It makes me feel [실제 감정].

**본문:** The main thing is [핵심 특징]. For example, [구체적인 모습/기능]. [같은 특징을 보여 주는 설명].

**마무리:** That's what makes [대상] special to me.

### H 틀 — 재활용·쇼핑·전화·여행 준비 등

**What:** I usually [행동] [언제/어디서].

**Why:** I do that because [이유].

**Feeling:** I feel [감정] when I do it.

**본문:** First, I [첫 행동]. Then, I [다음 행동]. What matters most to me is [과정 중 중요점].

**마무리:** That's how I usually [행동 요약].

### P 틀 — 어릴 때·최근·고장·처음 경험 등

**What:** [질문에 맞는 때], [핵심 사건이 일어남].

**Why:** [그 사건이 중요하거나 좋고 나빴던 구체적 이유].

**Feeling:** I felt [당시 감정].

**본문:** [필요한 배경]. [진행/내가 한 행동]. Eventually, [결과].

**마무리:** Looking back, [지금 느끼는 의미].

### C 틀 — 두 사람·두 장르 / 과거·현재

**What:** These days, [현재 상태]. / The main difference between A and B is [비교 기준].

**Why:** [현재 상태/차이에 대한 이유].

**Feeling:** I feel [내 감정·평가].

**본문:** In the past, [동일 기준의 과거]. Now, [현재]. / Both [공통점], but A [특징], while B [동일 기준의 차이].

**마무리:** That's why [변화에 대한 생각/선호].

### 우선 보충할 실제 내용

| 부족한 내용 | 최소 메모 | 연결 질문 |
|---|---|---|
| 영화·TV | 작품/장르 하나, 인물 하나, 시청 습관, 최근 시청 사건 | p.1·3 |
| 음악 | 실제 장소·기기·아티스트, 비교할 다른 장르, 라이브 경험 | p.2 |
| 집·가구 | 집 구조, 가사 하나, 어릴 때 집, 실제 가구와 문제 | p.5·13 |
| 여행 | 첫/어린 시절 여행, 준비 순서, 제주 사실 확인 | p.6·7·18 |
| 집에서 휴가 | 실제 휴가 연도와 한 사건, 누구와 무엇을 했는지 | p.8 |
| 인터넷·전화 | 사이트/영상, 첫 기기, 통화 습관, 실제 문제 | p.9·10 |
| 산업·시사 | 실제 회사명·개인 준비 경험, 설명 가능한 기사와 시점 | p.12 및 뉴스 문항 |
| 재활용 | 실제 분리 품목·순서, 한 번 겪은 문제 | p.14 |
| 식당·음식 | 실제 식당·메뉴, 주문 습관, 음식과 연결된 사건 | p.15·16 |
| 건강 | 구체적 운동 습관, 새로 시도한 실제 경험 | p.17 |
| 교통 | 어린 시절 수단, 직접 겪은 교통 문제 | p.19 |
| 모임·명절 | 최근/어린 시절 모임 하나, 준비에 참여한 일 | p.20·21 |
| 은행·호텔 | 실제 지점 업무·은행 문제, 호텔명과 투숙 경험 | p.22·23 |
| 예약·가족·날씨·패션 | 치과/미용실, 두 번째 인물, 악천후 사건, 실제 쇼핑 | p.24·26~28 |

## 7. 연습 순서

1. **핵심 경험부터:** P01 이어폰 → P02 하와이 → P03 음악 계기 → P04 예약 변경.
2. **같은 소재의 유형 바꾸기:** D02 음악 특징 → H01 음악 습관 → P03 시작 계기 → C03 변화.
3. **질문 맞춤 확인:** 장소·시간·대상·행동 중 질문이 지정한 조건에 밑줄을 긋습니다.
4. **MP만 먼저 말하기:** What → Why → Feeling을 메모 없이 말하고 본문으로 이어갑니다. 강의의 짧고 명확한 MP 취지를 따르되, 특정 초수를 채우기 위해 말하지 않습니다.
5. **표현은 필요한 만큼:** 답변당 1~3개부터 사용합니다. 이미 잘 말하고 있으면 필러를 더 넣지 않습니다. 외운 도입부보다 질문에 대한 첫 내용이 먼저 전달되게 합니다.

자기소개와 전화 롤플레이는 이 PDF의 질문 분류 대상이 아니므로 기존 자료에 남겨 두었습니다. PDF에 인쇄된 문항 빈도·등급·출제 번호를 공식 보장으로 사용하지 않습니다.
